...
import {
  ...
  editProduct,
  fetchProductById,
} from "../../store/vendor/product-slice";
import {
...
function VendorProductForm() {
  ...
  const categoryRequiresSubcategory = useMemo(
    () => (selectedCategory) => {
  ...
  const getSuggestedSpecifications = (
    selectedCategory,
    selectedSubcategory,
  ) => {
    if (
      selectedSubcategory &&
      specificationTemplatesBySubcategory[selectedSubcategory]
    ) {
      return specificationTemplatesBySubcategory[selectedSubcategory].map(
        (specification) => ({ ...specification }),
      );
    }
    if (
      selectedCategory &&
      specificationTemplatesByCategory[selectedCategory]
    ) {
      return specificationTemplatesByCategory[selectedCategory].map(
        (specification) => ({ ...specification }),
      );
    }
    return specificationTemplatesByCategory.default.map((specification) => ({
      ...specification,
    }));
  };
          ...
          return {
            ...
            customOnChange: (value) => {
              setFormData((prev) => ({
                ...
                subcategory: "",
                specifications:
                  prev.specifications?.length > 0
                    ? prev.specifications
                    : getSuggestedSpecifications(value, ""),
              }));
            },
          };
        }
        ...
        customOnChange: (value) => {
          setFormData((prev) => ({
            ...
            specifications:
              prev.specifications?.length > 0
                ? prev.specifications
                : getSuggestedSpecifications(prev.category, value),
          }));
        },
      };
    });
  ...
  const isFormValid = () => {
    ...
    const hasValidSpecifications = (formData.specifications || []).every(
      (specification) => {
        const hasName =
    ...
    return (
      hasRequiredFields &&
      (formData.specifications?.length ? hasValidSpecifications : true)
    );
  };
  ...
  const handleSubmit = (event) => {
    ...
    if (!canInteract) {
      toast.error(
        "Please upload an image first before filling the product details.",
      );
      return;
    }
    if (!isFormValid()) {
      toast.error(
        "Please complete all required product fields before submitting.",
      );
      return;
    }
    ...
    const payload = {
      ...
      keyFeatures:
        sanitizedSpecifications.length > 0
    ...
    submitAction
      .then((data) => {
        if (data.payload?.success) {
          ...
          toast.error(
            data.payload?.message || "Unable to save the product right now.",
          );
        }
      })
      .catch(() => {
  ...
  const fieldClasses =
    "h-11 w-full rounded-xl border border-slate-200 bg-white px-3.5 text-sm text-slate-900 placeholder:text-slate-400 shadow-sm transition focus:border-slate-400 focus:outline-none focus:ring-4 focus:ring-slate-100 disabled:cursor-not-allowed disabled:bg-slate-50 disabled:text-slate-400";
      ...
      case "select":
        return (
          <Select
            ...
            onValueChange={(next) => {
              if (typeof control.customOnChange === "function")
                control.customOnChange(next);
              else setFormData((prev) => ({ ...prev, [control.name]: next }));
            }}
            disabled={control.disabled}
          >
            <SelectTrigger
              className={`${fieldClasses} data-[placeholder]:text-slate-400`}
            >
              ...
              {control.options?.length > 0
                ? control.options.map((option) => (
                    <SelectItem
                      key={option.id}
                      value={option.value}
                      className="cursor-pointer rounded-lg px-3 py-2 text-sm text-slate-700 focus:bg-slate-100"
                    >
                      {option.label}
                    </SelectItem>
                  ))
      ...
      default:
        return (
          <Input
            ...
            onChange={(event) => {
              if (typeof control.customOnChange === "function")
                control.customOnChange(event.target.value);
              else
                setFormData((prev) => ({
                  ...prev,
                  [control.name]: event.target.value,
  ...
  const primaryControls = detailControls.filter(
    (c) => c.name === "name" || c.name === "description",
  );
  const secondaryControls = detailControls.filter(
    (c) => c.name !== "name" && c.name !== "description",
  );
          ...
          <div className="space-y-8 lg:col-span-7">
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
              <div className="mb-6">
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Basic information shoppers will see.
                </p>
              </div>
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
              <div className="mb-6 flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-500">
                    Add details like RAM, size, color, or processor.
                  </p>
                </div>
                <Button
                  type="button"
              ...
              {formData.specifications?.length > 0 ? (
                <div className="space-y-3">
                  {formData.specifications.map((specification, index) => (
                    <div
                      key={`specification-row-${index}`}
                      className="grid grid-cols-[1fr_1fr_auto] gap-3 rounded-2xl border border-slate-200 bg-slate-50/60 p-3"
                    >
                      <Input
                ...
                <p className="rounded-2xl border border-dashed border-slate-200 bg-slate-50/60 px-4 py-6 text-center text-sm text-slate-500">
                  No specifications added yet. Add a few to help shoppers
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
              <div className="mb-6">
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Required for accurate shipping cost calculation.
                </p>
              </div>
              ...
              <div className="space-y-5">
                ...
                <div>
                  <h3 className="mb-4 block text-sm font-medium text-slate-700">
                    Dimensions (Optional)
                  </h3>
