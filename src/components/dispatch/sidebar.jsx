...
const MenuItems = ({setOpen}) => {
  ...
  {
    dispatchSidebarMenuItems.map(menuItem => <div key={menuItem.id} onClick={() => {navigate(`${menuItem.href}`); setOpen ? setOpen(false) : null}} className='flex items-center px-3 py-2 rounded-md gap-2 hover:bg-gray-700 hover:text-white'>
      {menuItem.icon}
  ...
  return (
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetContent side='left' className='w-64 bg-white border-r p-6 lg:hidden flex flex-col'>
        <div className='flex flex-col gap-2 h-full'>
          <SheetHeader className='border-b pb-4 mb-4'>
            ...
            <House />
            <span>Dispatch Panel</span></SheetTitle>
          </SheetHeader>
      ...
      <aside className='w-64 hidden border-r border-gray-200 p-6 lg:flex flex-col'>
        ...
        <House />
           <h1 className='text-xl font-bold'>Dispatch Panel</h1>
        </div>
        <MenuItems setOpen={setOpen} />
      </aside>
    </Fragment>
